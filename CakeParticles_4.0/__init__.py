...
class CakeParticlesPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        bake_box.label(text='Press N in the timeline to open the EDIT panel', icon='BRUSH_CURVES_CUT')
...
class BakeParticlesOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        # bake_step = context.window_manager.bake_step
        main(context)
        return {"FINISHED"}
...
class SimplifyAnimationPanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        s_box.label(text="Hover or click over the timeline to refresh don't spam the button", icon='RESTRICT_SELECT_OFF')
